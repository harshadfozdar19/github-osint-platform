...
By default, content analysis fetches a bounded, prioritized set of files per repo via the GitHub REST API (`SCAN_MAX_FILES_PER_REPO`, default 12) — a deliberate trade-off to stay within GitHub's rate limit when scanning many repos. For deeper coverage without spending API quota, **clone-based scanning** (`ENABLE_CLONE_SCAN`, off by default) shallow-clones eligible repos over git's own transport instead — which isn't subject to the REST rate limit at all — and scans the full working tree locally (skipping `node_modules`, `dist`, `vendor`, and similar directories). It's opt-in and fails closed: any problem (git unavailable, clone timeout, repo over `CLONE_SCAN_MAX_REPO_SIZE_KB`) just falls back to the REST-based fetch, so it can be disabled at any time without affecting the rest of the pipeline.
